...
// core/webdav free of any dependency on the app composer.
...
func RegisterTSHooks(host HostBindings, src Source) TSHooks {
	hooks := TSHooks{
		...
		BeforeMove:   host.Point(pointName(src.Slug, "beforeMove")),
	...
	host.RegisterLoaderBinder(func(loader *sobek.Runtime, compile jsvm.Compiler, _ *pocketbase.PocketBase) error {
		// webdavHook({ beforeWrite(e) {...}, filterList(e) {...} })
		//
		...
		register := func(handlers map[string]string) error {
			for _, name := range hookPointNames {
				...
				if err != nil {
					return fmt.Errorf("webdavHook: %s: %w", name, err)
				}
				host.AddHandler(pointName(slug, name), fn)
			}
			for name := range handlers {
				if !isKnownHookName(name) {
					return fmt.Errorf("webdavHook: unknown hook %q (known: %v)", name, hookPointNames)
				}
			}
			return nil
		}
		return loader.Set("webdavHook", register)
	})
