...
// Platform is one of the two native targets exported for OTA updates.
...
func parseExportMetadata(distDir string, platform Platform, buildID, runtimeVersion string) (BundleMeta, error) {
...
func (p Pipeline) ExportNativeBundles(sink ProgressSink, appDir, buildID, runtimeVersion string) ([]BundleMeta, error) {
	...
	for i, p2 := range platforms {
		outDir := filepath.Join(appDir, fmt.Sprintf("dist-%s", p2))
		...
		if cmdOut, err := p.runExportWithProgress(sink, lo, hi, step, appDir,
			"--platform", string(p2), "--source-maps", "external", "--output-dir", outDir); err != nil {
			return nil, fmt.Errorf("expo export %s: %v: %s", p2, err, cmdOut)
		}
		bm, err := parseExportMetadata(outDir, p2, buildID, runtimeVersion)
		if err != nil {
			return nil, fmt.Errorf("parse %s export: %w", p2, err)
		}
		bm.DistDir = outDir
		...
		p.uploadBundleSourcemaps(sink, outDir, runtimeVersion, bm.BundleID)
		out = append(out, bm)
	}
	return out, nil
}
...
// from the host's ConfigValue (the system-wide source of truth), not env vars.
func (p Pipeline) uploadBundleSourcemaps(sink ProgressSink, outDir, runtimeVersion, bundleID string) {
	authToken := p.configValue("sentry.auth_token")
	if authToken == "" {
		sink.Logf("sourcemap upload skipped for %s (sentry.auth_token unset)", bundleID)
		return
	}
	...
	// Configurable via host settings but defaulted to the known argosity/tinycld
	// project so a standard deploy needs only the auth token.
	org := orDefault(p.configValue("sentry.org"), "argosity")
	project := orDefault(p.configValue("sentry.project"), "tinycld")
	...
	if err != nil {
		...
		sink.Logf("sourcemap upload FAILED for %s (release=%s): %v: %s", bundleID, release, err, out)
		return
	}
	sink.Logf("uploaded sourcemaps for %s (release=%s)", bundleID, release)
}
