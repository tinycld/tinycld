import {
    ...
    extractDroppedFiles,
    isFileDrag,
} from '@tinycld/core/lib/file-drop/file-drop-controller'
...
describe('isFileDrag', () => {
    it('is true when the drag advertises the Files type', () => {
        expect(isFileDrag({ types: ['Files'] })).toBe(true)
        expect(isFileDrag({ types: ['text/uri-list', 'Files'] })).toBe(true)
    })

    it('is false for in-page text/element drags', () => {
        expect(isFileDrag({ types: ['text/plain'] })).toBe(false)
        expect(isFileDrag({ types: [] })).toBe(false)
    })

    it('is false when dataTransfer or types is missing', () => {
