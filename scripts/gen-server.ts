...
export interface ServerPkg {
    ...
    // The package's tenant entry consumes router-managed mail listeners
    // (manifest server.mailListeners) — emitted as RegisterTenantWithListeners
    // in the tenant registrar.
...
// buildTenantPackageExtensionsGo emits the TENANT-mode registrar — the
// dual-mode binary's counterpart of registerPackageExtensions. Registration is
// unconditional: a per-org build links exactly the org's package set, so the
// artifact itself is the gate (no runtime slug filtering, unlike the pinned
// tenantpkgs menu this replaces for hosted orgs). A package that declared
// server.mailListeners gets the router-managed mail sockets injected via its
// own TenantListeners type; everything else registers its plain RegisterTenant
// composition.
export function buildTenantPackageExtensionsGo(pkgs: ServerPkg[]): string {
    if (pkgs.length === 0) {
        return [
            '// Code generated by app/scripts/generate.ts. DO NOT EDIT.',
            'package main',
    ...
    const imports = pkgs.map(p => `\t${slugToIdentifier(p.slug)} "${p.module}"`)
    const calls = pkgs.map(p => {
        const id = slugToIdentifier(p.slug)
        if (!p.mailListeners) return `\t${id}.RegisterTenant(app)`
        return [
            `\t${id}.RegisterTenantWithListeners(app, ${id}.TenantListeners{`,
            '\t\tIMAP:       ex.Mail.IMAP,',
    ...
    return [
        ...
        ')',
        '',
        'func registerTenantPackageExtensions(app *pocketbase.PocketBase, ex tenantmain.Extras) {',
        ...calls,
        '}',
        '',
    ].join('\n')
