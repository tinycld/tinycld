#!/usr/bin/env bash
# End-to-end local driver for the OTA CRASH-ROLLBACK native E2E (iOS).
#
# The crash-rollback variant of run-ota-dry-run.sh. Instead of installing the
# happy-path `todo` package, it installs `@tinycld/calendar-slots` (an external
# github: package, via the in-app installer UI driven by
# tests/install/calendar-slots-install.spec.ts), which mints a `build-<ts>-ios`
# OTA bundle. It then boots a Release sim and runs the rollback assertion
# (test:e2e:ota:rollback), which races TWO outcomes and asserts which happened:
#   - HEALTHY: the app reloads into the new bundle (and the booking tables exist).
#   - ROLLBACK: the app crash-loops the new bundle and reverts; the server records
#     a pkg_bad_bundle row whose last_error carries a CAPTURED reason (an empty or
#     generic last_error is a FAILURE — the gap the manual repro hit).
# Select which outcome is the pass with OTA_E2E_EXPECT=healthy|rollback (default
# rollback, the case we are chasing).
#
# Steps mirror run-ota-dry-run.sh:
...
#   9. Run the rollback assertion (`pnpm run test:e2e:ota:rollback`).
#
...
#   SERVER_PORT=7091   Host port the container serves on (loopback). Defaults to a
#                      DISTINCT port from the dry-run so a KEEP=1 dry-run container
#                      and a KEEP=1 rollback container can coexist.
#   CONTAINER=<name>   Container name (distinct default from the dry-run, ditto).
#   PKG_SPEC=<spec>    The package to install (default the calendar-slots github repo).
#   OTA_E2E_EXPECT=healthy|rollback  Which terminal outcome is the pass (default rollback).
...
CONTAINER="${CONTAINER:-tinycld-ota-rollback-server}"
SERVER_PORT="${SERVER_PORT:-7091}"
...
LOG_DIR="${SCRIPT_DIR}/ota-crash-rollback-logs"
...
log() { printf '[ota-rollback] %s\n' "$*"; }
die() { printf '[ota-rollback] FAIL: %s\n' "$*" >&2; exit 1; }
...
boot_container() {
    MOUNT_ROOT="$(mktemp -d -t tinycld-ota-rollback-mounts.XXXXXX)"
...
# ---------------------------------------------------------------------------
# Step 5 — install calendar-slots (mints the ios bundle via the installer UI)
# ---------------------------------------------------------------------------
...
mint_ios_bundle() {
    ...
    (
        ...
        RUN_CALSLOTS_INSTALL_TEST=1 \
        CI=true FORCE_COLOR=0 \
        pnpm exec playwright test calendar-slots-install.spec.ts --reporter=line -g "${grep_expr}"
    ) > "${INSTALL_LOG}" 2>&1 || {
...
# ---------------------------------------------------------------------------
# Step 9 — run the rollback assertion (races healthy-flip vs reported-rollback)
# ---------------------------------------------------------------------------
assert_rollback() {
    log "running the OTA crash-rollback assertion (expecting ${OTA_E2E_EXPECT:-rollback})…"
    (
        cd "${APP_DIR}"
    ...
    ) || die "OTA crash-rollback assertion failed (see above + ${CONTAINER_LOG})"
    log "✅ OTA crash-rollback run PASSED"
}
