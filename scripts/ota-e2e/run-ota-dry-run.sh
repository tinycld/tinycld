...
build_release_sim() {
    ...
    # ios-simulator.sh --prod builds Release (embedded bundle, OTA loader active)
    # and installs+launches on the sim. We don't pass a server addr (the app reads
    # none); the cached-address seed below points it at our server.
    (cd "${APP_DIR}" && ./scripts/ios-simulator.sh --prod) \
        || die "ios-simulator.sh --prod failed (Release build/boot)"
}
