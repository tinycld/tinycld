...
- **Load a parent with its children in one request** through a back-relation. The generated `Schema` names every back-relation (`boards_comments_via_card`); declare it under the parent's `relations` and read the parent through `parent.fetchRelations('<child>_via_<fk>')` on an **on-demand** parent: PocketBase returns the children with the parent, pbtsdb files them into their own on-demand collection and records that the subset for that parent is complete, and an include whose only predicate is that single foreign-key equality (`eq(comment.card, card.id)`) is served from the store — no request per child collection. A second predicate (`and(...)`, another field) turns the include back into a server request, so filter the rest in JS. PocketBase caps a back-relation expand at 1000 rows per parent; past that the include falls back to its own filtered request. `boards/tinycld/boards/collections.ts` and `hooks/useCardDetail.ts` are the reference.
...
- **Core must not name a package** — no slug, scope, collection or route of any feature in `core/`, `app/`, `cli/`, `scripts/` or `server/`. When core needs to know something about a package, the package registers it from its Go `Register(app)` (`oauth.RegisterPackage`, `search.RegisterSources`, `quota.RegisterSources`, `offboard.RegisterReassignable`, …) and core reads the registry at runtime. `pnpm run check:core-isolation` (part of `pnpm run checks`) derives the installed packages from the workspace and fails on any reference; its allowlist is a shrinking debt register, not a place to add to. Full rationale and the registry table: `docs/packages.md`, "Registries: how core learns what a package provides".
...
- **UI Framework: Gluestack UI + Uniwind (Tailwind v4 for React Native)**
  ...
  - **Dialogs: use `Dialog` from `@tinycld/core/ui/dialog`** (there are no raw modal parts to reach for). It gives every dialog the same title row (with a close button), a `Dialog.Body` that scrolls once the dialog reaches 90% of the viewport, and a `Dialog.Footer` whose buttons stay put (`Dialog.CancelButton`, `Dialog.ActionButton`). Anything that must not scroll — a segmented control, a search field — goes between the two as an ordinary child. `ConfirmDialog` and `PromptDialog` are built on it. Do not hand-roll a header, a `ScrollView` with a fixed `maxHeight`, or a button row inside `ModalContent` — that layout is exactly what kept getting cut off.
  - Every surface animates in with a **predefined** Reanimated animation on purpose: a custom keyframe (`ZoomIn.withInitialValues(...)`) makes Reanimated pin the element's width and height on web after the animation, so a dialog can never grow with its content again.
  - **Overlays** — every floating surface is one of `Dialog` (`core/ui/dialog`), `Popover` (`core/ui/popover`), `Menu` (`core/ui/menu`) or `Sheet` (`core/ui/sheet`), all on one internal engine (`core/ui/overlay`, never imported by a package). `Menu` takes a `trigger` element and prop-based rows (`Menu.Item label icon onSelect`, `Menu.CheckboxItem`, `Menu.Sub`, `Menu.Section`, `Menu.Separator`, `Menu.Custom`); a `Popover` is the same anchored surface with arbitrary content; both become sheets on the mobile breakpoint. Full reference: `docs/overlays.md`.
  - Layout: `View` + `className="flex-row"` replaces XStack; plain `View` is column by default (replaces YStack).
  - **Toolbars: use `ResponsiveToolbar`** (`core/components/ResponsiveToolbar`) for any row of actions — a screen header's controls, a selection bar, a formatting strip. It measures its items with `onLayout` on web and native alike and folds whatever does not fit into a ⋮ "More actions" `Menu`, so a row never overflows or scrolls sideways. Items are data: `button` (icon + label + onPress), `menu` (rows in a `Menu`, optionally with the caller's own `trigger`), `separator`, `spacer` (a flexible gap; what follows sits at the right edge and folds first), and `custom` (any element). A `custom` item folds according to its `overflow`: `{ label, onPress }` becomes a menu row, `{ label, children }` a submenu of `Menu.*` rows, `'hide'` simply drops (status chrome), and omitting `overflow` pins the item to the row; a pinned title or breadcrumb that truncates declares `minWidth`, the floor it may shrink to before anything folds. `rightItems` are pinned at the right edge; `moreMenu` holds permanent `Menu.*` rows (a board's or card's own actions) so the More button is always there and overflowed items land above them; `moreTrigger` lets the row keep its own button style. A row that already has a "…" menu becomes the toolbar's `moreMenu`; a control whose popover holds a widget rather than rows (a filter panel) stays pinned, since a folded trigger has nothing to anchor to. Leave alone: menubars, navigation tabs, wrapping filter chips, content scrollers (attachments, sheet tabs), and single per-row kebab menus.
