...
/** The generator's own output, never a linked feature. */
const GENERATED_NAMES = ['app-generated']

function linkedPackageNames(): string[] {
    const configPath = path.join(APP_DIR, 'tinycld.config.ts')
    const source = fs.readFileSync(configPath, 'utf8')
    const names = [...source.matchAll(/@tinycld\/([a-z0-9-]+)/g)].map(m => m[1])
    return [...new Set(names)].filter(name => !GENERATED_NAMES.includes(name))
}

describe.runIf(process.env.CI)('linked packages (CI lean shell)', () => {
    it('links nothing beyond core and the E2E stubs', () => {
        const unexpected = linkedPackageNames().filter(
            name => name !== 'core' && !STUB_SLUGS.includes(name)
        )
    ...
    it('reads a config that actually lists core (guards the parse)', () => {
        // Without this, a moved file or changed config shape yields an empty
        // list and the check above passes vacuously. Core is asserted rather
        // than a stub because only the E2E job scaffolds stubs — unit tests run
        // in the checks job, whose assembly is app + core alone.
        expect(linkedPackageNames()).toContain('core')
    })
})
